...
export class BooksApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param review 
     */
    public async bookControllerFindUserBookReview(bookId: string, isRead?: boolean, rate?: number, review?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;
        ...
        // Query Params
        if (isRead !== undefined) {
            requestContext.setQueryParam("isRead", ObjectSerializer.serialize(isRead, "boolean", ""));
        }
     ...
     * @param review 
     */
    public async bookControllerFindUserBookReviewCount(bookId: string, isRead?: boolean, rate?: number, review?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;
    ...
    public async bookControllerFindUserBookReviews(bookId: string, page: number, limit: number, _options?: Configuration): Promise<RequestContext> {
        ...
        // Path Params
        const localVarPath = '/v1/user/books/review';
     ...
     * @param limit 
     */
    public async bookControllerFindUserBooks(page: number, limit: number, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'page' is not null or undefined
        if (page === null || page === undefined) {
            throw new RequiredError("BooksApi", "bookControllerFindUserBooks", "page");
        }


        // verify required parameter 'limit' is not null or undefined
        if (limit === null || limit === undefined) {
            throw new RequiredError("BooksApi", "bookControllerFindUserBooks", "limit");
        }


        // Path Params
        const localVarPath = '/v1/user/books';
...
export class BooksApiResponseProcessor {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async bookControllerFindUserBookReviewCountWithHttpInfo(response: ResponseContext): Promise<HttpInfo<UserBookReviewCountResponseDto >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: UserBookReviewCountResponseDto = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "UserBookReviewCountResponseDto", ""
            ) as UserBookReviewCountResponseDto;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
        }
        if (isCodeInRange("401", response.httpStatusCode)) {
            throw new ApiException<undefined>(response.httpStatusCode, "Unauthorized", undefined, response.headers);
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to bookControllerFindUserBooks
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async bookControllerFindUserBooksWithHttpInfo(response: ResponseContext): Promise<HttpInfo<UserBooksResponseDto >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: UserBooksResponseDto = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
