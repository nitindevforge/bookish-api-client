...
export class BooksApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param user 
     */
    public async bookControllerFindTopUserSetupBooks(rate: number, page: number, limit: number, search?: string, user?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'rate' is not null or undefined
        if (rate === null || rate === undefined) {
            throw new RequiredError("BooksApi", "bookControllerFindTopUserSetupBooks", "rate");
        }


        // verify required parameter 'page' is not null or undefined
        if (page === null || page === undefined) {
            throw new RequiredError("BooksApi", "bookControllerFindTopUserSetupBooks", "page");
        }


        // verify required parameter 'limit' is not null or undefined
        if (limit === null || limit === undefined) {
            throw new RequiredError("BooksApi", "bookControllerFindTopUserSetupBooks", "limit");
        }
        ...
        // Path Params
        const localVarPath = '/v1/top/books/setup';
...
export class BooksApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to bookControllerFindTopUserSetupBooks
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async bookControllerFindTopUserSetupBooksWithHttpInfo(response: ResponseContext): Promise<HttpInfo<BooksReviewResponseDto >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: BooksReviewResponseDto = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
