...
export class PaymentApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param paymentPayloadDto 
     */
    public async paymentControllerDeletePayment(paymentPayloadDto: PaymentPayloadDto, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'paymentPayloadDto' is not null or undefined
        if (paymentPayloadDto === null || paymentPayloadDto === undefined) {
            throw new RequiredError("PaymentApi", "paymentControllerDeletePayment", "paymentPayloadDto");
        }
...
export class PaymentApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to paymentControllerDeletePayment
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async paymentControllerDeletePaymentWithHttpInfo(response: ResponseContext): Promise<HttpInfo<PaymentDeleteResponseDto >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: PaymentDeleteResponseDto = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "PaymentDeleteResponseDto", ""
            ) as PaymentDeleteResponseDto;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
        }
        if (isCodeInRange("401", response.httpStatusCode)) {
            throw new ApiException<undefined>(response.httpStatusCode, "Unauthorized", undefined, response.headers);
