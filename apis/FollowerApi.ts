...
export class FollowerApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param search 
     */
    public async followerControllerMyFriends(page: number, limit: number, search?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'page' is not null or undefined
        if (page === null || page === undefined) {
            throw new RequiredError("FollowerApi", "followerControllerMyFriends", "page");
        }


        // verify required parameter 'limit' is not null or undefined
        if (limit === null || limit === undefined) {
            throw new RequiredError("FollowerApi", "followerControllerMyFriends", "limit");
        }



        // Path Params
        const localVarPath = '/v1/myFriends';
...
export class FollowerApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to followerControllerMyFriends
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async followerControllerMyFriendsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<MyFriendsResponseDto >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: MyFriendsResponseDto = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "MyFriendsResponseDto", ""
            ) as MyFriendsResponseDto;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
        }
        if (isCodeInRange("401", response.httpStatusCode)) {
            throw new ApiException<undefined>(response.httpStatusCode, "Unauthorized", undefined, response.headers);
